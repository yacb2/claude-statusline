...
The script keeps one account-wide cache (`~/.claude/rate-limits-cache.json`) and merges **per window, from the data itself**: same `resets_at` → higher `used_percentage` wins (usage is cumulative); expired → dropped; two live windows with different `resets_at` → the snapshot taken later wins, dated by the last real assistant response in the session's transcript (API-error entries are skipped). A natural rollover never yields two live windows, so that case means the server replaced one — and `resets_at` order says nothing then: the 2026-09-01 account-wide reset produced an *earlier* `resets_at`. The transcript mtime is never used, so an idle session cannot publish a stale snapshot over a live one.
